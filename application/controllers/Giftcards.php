...
class Giftcards extends Secure_area implements Idata_controller
{
	...
	{
		return array(lang('giftcards_giftcard_number'),lang('giftcards_card_value'), lang('common_inactive'));
	}
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				for($k = 2;$k<=$num_rows; $k++)
				{
					...
					$value = $sheet->getCellByColumnAndRow(1, $k)->getValue();
					...
					$inactive = $sheet->getCellByColumnAndRow(2, $k)->getValue();
