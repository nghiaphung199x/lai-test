...
class Languagecheck extends MY_Controller {
	...
	function index($disable_white_list = 0, $fix_missing_keys = 0, $fix_matching_english_keys = 0)
	{
		...
		$this->output('h1', '4Biz by LifeTek - Language file checking and validation');
								...
								elseif($lang[$key] == $lang_ref[$key])
								{
									if (!$disable_white_list)
									{
										$white_list = array(
											...
											'locations_credit_card_processor','locations_emv_merchant_id','locations_braintree_public_key','locations_braintree_private_key','locations_mailchimp_api_key','locations_stripe_private','locations_name','locations_com_port',
											'reports_subtotal','sales_unredeem','sales_type','sales_entry_method','sales_undeleted_voided'
										);
									}
									else
									{
