...
function get_item_kits_barcode_data($item_kits_ids)
{
	...
	foreach ($item_kit_ids as $item_kit_id)
	{
		...
		if($CI->config->item('barcode_price_include_tax'))
		{
			...
			{
				$result[] = array('name' => (!$hide_prices ? '<span style="font-size:12pt;">'.to_currency($item_kit_price).'</span>: ' : '').$item_kit_info->name, 'id'=> 'KIT ' .number_pad($item_kit_id, 10));
			}
			else
			{				
				$result[] = array('name' =>(!$hide_prices ? '<span style="font-size:12pt;">'.to_currency(get_price_for_item_kit_including_taxes($item_kit_id,$item_kit_price)).'</span>: ': '').$item_kit_info->name, 'id'=> 'KIT ' .number_pad($item_kit_id, 10));
	  	 	}
	  }
	  else
	  {
		if ($item_kit_info->tax_included)
		{
		    $result[] = array('name' =>(!$hide_prices ? '<span style="font-size:12pt;">'.to_currency(get_price_for_item_kit_excluding_taxes($item_kit_id, $item_kit_price)).'</span>: ' : '').$item_kit_info->name, 'id'=> 'KIT ' .number_pad($item_kit_id, 10));
		}
		else
		{
	    	$result[] = array('name' =>(!$hide_prices ? '<span style="font-size:12pt;">'.to_currency($item_kit_price).'</span>: ' : '').$item_kit_info->name, 'id'=> 'KIT ' .number_pad($item_kit_id, 10));
