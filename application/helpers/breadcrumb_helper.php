...
function create_breadcrumb()
{
    ...
    } elseif ($ci->uri->segment(1) == 'groups') {

        if ($ci->uri->segment(2) == NULL) //Main page
        {
            $groups_home_link = create_current_page_url(lang('module_groups'));
        } else {
            $groups_home_link = '<a tabindex = "-1" href="' . site_url('groups') . '">' . lang('module_groups') . '</a>';
        }

        $return .= $groups_home_link;

        if ($ci->uri->segment(2) == 'view') {
            if ($ci->uri->segment(3) == -1) {
                $return .= create_current_page_url(lang('groups_new'));
            } else {
                $return .= create_current_page_url(lang('groups_update'));
            }
        } elseif ($ci->uri->segment(2) == 'excel_import') {
            $return .= create_current_page_url(lang('groups_import_groups_from_excel'));
        }

    } elseif ($ci->uri->segment(1) == 'departments') {

        if ($ci->uri->segment(2) == NULL) //Main page
        {
            $departments_home_link = create_current_page_url(lang('module_departments'));
        } else {
            $departments_home_link = '<a tabindex = "-1" href="' . site_url('departments') . '">' . lang('module_departments') . '</a>';
        }

        $return .= $departments_home_link;

        if ($ci->uri->segment(2) == 'view') {
            if ($ci->uri->segment(3) == -1) {
                $return .= create_current_page_url(lang('departments_new'));
            } else {
                $return .= create_current_page_url(lang('departments_update'));
            }
        } elseif ($ci->uri->segment(2) == 'excel_import') {
            $return .= create_current_page_url(lang('departments_import_departments_from_excel'));
        }
