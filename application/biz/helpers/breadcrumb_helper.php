...
function create_breadcrumb()
{
	...
	if ($ci->uri->segment(1) == 'customers')
	{
		...
		elseif($ci->uri->segment(2) == 'manage_sms')
		{
			$return.=create_current_page_url(lang('module_customers_sms'));
		}
		elseif($ci->uri->segment(2) == 'view_sms')
		{
			if ($ci->uri->segment(3) == -1)
			{
  				$return.=create_current_page_url(lang('customers_sms_new'));
			}
			else
			{
