     	...
     	CKEDITOR.replace('" . $data['id'] . "', {";
    		...
    		if(isset($data['config'])) {
	    		foreach($data['config'] as $k=>$v) {
	    			...
	    			if($k !== array_pop((array_slice(array_keys($data['config']), -1)))) {
						$return .= ",";
					}
	    		} 
    		}   			
	    ...
	    foreach($data['styles'] as $k=>$v) {
	    	...
	    	if(isset($v['styles'])) {
	    		foreach($v['styles'] as $k2=>$v2) {
					...
					if($k2 !== array_pop((array_slice(array_keys($v['styles']), -1)))) {
						 $return .= ",";
					}
	    		} 
    		} 
	    	...
	    	if($k !== array_pop((array_slice(array_keys($data['styles']), -1)))) {
				$return .= ',';
			}	    	
