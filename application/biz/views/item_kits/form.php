				...
				<table id="item_kit_items" class="table table-bordered table-striped text-success text-center">
					...
					?>
						<tr>
							...
							<td><?php echo (int) $this->Item_location->get_location_quantity($item_kit_item->item_id); ?></td>
							<td>
...
function countAvailablePackages()
{
	...
	_data['items'] = items;
	coreAjax.call(
		'<?php echo site_url("item_kits/count_available_kits");?>',
		_data,
		function(response)
		{
			if(response.success)
			{
				$('#numberOfAvailableKits span').text('(Tối đa được '+ response.available_kits +' gói)'); 
			}
		}
	);
}
