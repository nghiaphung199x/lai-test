	...
	<div id="pdf_header">
		...
		<div class="w100">
			<div id="pdf_short_info" class="w150px">
				...
				<p>
					Ngày: <span><?php echo date(get_date_format(), strtotime($transaction_time)); ?></span>
				</p>
			</div>
		</div>
	</div>
...
$stt ++;
				...
				<?php } ?>
					...
					echo lang ( 'common_total_money' ) . ': ';
					echo NumberFormatToCurrency ( abs ( $total_money ) );
					?></td>
				</tr>
				<?php
...
$total_tax = 0;
						...
						?>
						 	<tr>
					<td colspan="8" class="border-bottom border-top text-bold"><?php
						
echo '1111111' . lang ( 'common_tax' ) . ': ';
						echo NumberFormatToCurrency ( abs ( $total_tax ) );
						?></td>
				</tr>
					<?php }else {?>
							<?php
...
foreach ( $taxes as $name => $value ) {
				...
				<?php } ?>
                                        <tr>
					<td colspan="9"
						style="border-left: none; border-right: none; border-bottom: none;"></td>
				</tr>
			</tbody>
		</table>
	</div>
	...
	<div>
		<p>
			Số tiền viết bằng chữ: <span><?php echo getStringNumber((int)$total)?></span>
		</p>
	</div>
