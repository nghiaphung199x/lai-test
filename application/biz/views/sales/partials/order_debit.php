...
<div id="pdf_content">
	...
	<div id="pdf_customer">
		...
		<p>Địa chỉ kho: <span><?php echo nl2br($this->Location->get_info_for_key('address', isset($override_location_id) ? $override_location_id : FALSE)); ?></span></p>
                <p>Tổng nợ cũ: <span><?php echo number_format(to_currency_no_money($customer_balance_for_sale_before)); ?></span></p>
	</div>	
	<div class="w100 clb">
		<table id="pdf_tbl_items" class="w100">
			<tbody>
				...
				?>
					...
					<tr>
						...
						<td></td>
                                                ...
                                                <td><?php echo number_format(to_currency_no_money($item['price'])); ?></td>
						<td><?php echo to_quantity($item['discount']);?></td>
                                                <td><?php echo to_quantity($item['tax_included']);?></td>
                                                <td><?php echo number_format(to_currency_no_money($item['price']*$item['quantity']-$item['price']*$item['quantity']*$item['discount']/100)); ?></td>
					</tr>
				...
				<tr>
                                    <td colspan="9" class="border-bottom text-bold"><?php echo lang('common_total').': '; 
                                        echo $this->config->item('round_cash_on_sales') && $is_sale_cash_payment ? number_format(to_currency_no_money(round_to_nearest_05($total))) : number_format(to_currency_no_money($total)); ?></td>
				</tr>
					...
					?>
					<tr>
                                            <td colspan="9"  class="border-bottom border-top text-bold"><?php echo lang('common_tax').': ';  echo number_format(to_currency_no_money($total_tax)); ?></td>
					</tr>
				<?php }else {?>
					<?php foreach($taxes as $name=>$value) { ?>
						<tr>
                                                    <td colspan="9"  class="border-bottom border-top text-bold"><?php echo $name.': ';  echo number_format(to_currency_no_money($value)); ?></td>
						</tr>
					<?php }; ?>
				<?php } ?>
				...
				<tr>
					...
					<?php } 
                                        echo $this->config->item('round_cash_on_sales') && $payment['payment_type'] == lang('common_cash') ?  ': '.  number_format(to_currency_no_money(round_to_nearest_05($payment['payment_amount']))) : ': '.  number_format(to_currency_no_money($payment['payment_amount'])); ?></td>
				</tr>
				<?php } ?>
					...
					<?php if (strpos($payment['payment_type'], lang('common_giftcard'))!== FALSE) {?>
						...
						<tr>
							<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('sales_giftcard_balance'); 
                                                        ...
                                                        echo number_format(to_currency_no_money($this->Giftcard->get_giftcard_value(end($giftcard_payment_row)))); ?></td>
						</tr>
					<?php }?>
				<?php }?>
				...
				<?php if ($amount_change >= 0) {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_change_due').': '; 
                                                echo $this->config->item('round_cash_on_sales')  && $is_sale_cash_payment ? number_format(to_currency_no_money(round_to_nearest_05($amount_change))) : number_format(to_currency_no_money($amount_change)); ?></td>
					</tr>
				<?php } else { ?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_amount_due').': '; 
                                                echo $this->config->item('round_cash_on_sales')  && $is_sale_cash_payment ?  number_format(to_currency_no_money(round_to_nearest_05($amount_change * -1))) : number_format(to_currency_no_money($amount_change * -1)); ?></td>
					</tr>
				<?php } ?>
				...
				<?php if (isset($customer_balance_for_sale) && $customer_balance_for_sale !== FALSE && !$this->config->item('hide_store_account_balance_on_receipt')) {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold">Tổng nợ cuối: 
                                                    <?php echo number_format(to_currency_no_money($customer_balance_for_sale)); ?></td>
					</tr>
				<?php } ?>
