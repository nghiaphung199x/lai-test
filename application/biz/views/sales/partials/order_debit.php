...
<div id="pdf_content">
	...
	<div id="pdf_customer">
		...
		<p>Địa chỉ kho: <span><?php echo nl2br($this->Location->get_info_for_key('address', isset($override_location_id) ? $override_location_id : FALSE)); ?></span></p>
                <p>Tổng nợ cũ: <span><?php echo NumberFormatToCurrency($customer_balance_for_sale_before); ?></span></p>
	</div>	
		...
		<table id="pdf_tbl_items" class="w100 table">
			<tbody>
				...
				<tr>
					...
					?>
					<tr>
                                            <td colspan="9"  class="border-bottom border-top text-bold"><?php echo lang('common_tax').': ';  echo NumberFormatToCurrency($total_tax); ?></td>
					</tr>
				<?php }else {?>
					<?php foreach($taxes as $name=>$value) { ?>
						<tr>
                                                    <td colspan="9"  class="border-bottom border-top text-bold"><?php echo $name.': ';  echo NumberFormatToCurrency($value); ?></td>
						</tr>
					<?php }; ?>
				<?php } ?>
                                ...
                                <tr>
                                    <td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_total').': '; 
                                        echo $this->config->item('round_cash_on_sales') && $is_sale_cash_payment ? NumberFormatToCurrency(round_to_nearest_05($total)) : NumberFormatToCurrency($total); ?></td>
				</tr>
				<?php foreach($payments as $payment_id => $payment) { ?>
				<tr>
                                    <td colspan="9" class="border-bottom border-top text-bold"><?php
					...
					<?php } else { ?>
						<?php $splitpayment=explode(':',$payment['payment_type']); echo $splitpayment[0];
                                                if($splitpayment[0]==lang('common_store_account'))$amount_money +=$payment['payment_amount'];
                                                ?>
					<?php }
                                        echo $this->config->item('round_cash_on_sales') && $payment['payment_type'] == lang('common_cash') ?  ': '.  NumberFormatToCurrency(round_to_nearest_05($payment['payment_amount'])) : ': '.  NumberFormatToCurrency($payment['payment_amount']); ?></td>
				</tr>
				<?php } ?>
				...
				<?php if (isset($customer_balance_for_sale) && $customer_balance_for_sale !== FALSE && !$this->config->item('hide_store_account_balance_on_receipt')) {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold">Tổng nợ cuối: 
                                                    <?php echo NumberFormatToCurrency($customer_balance_for_sale); ?></td>
					</tr>
				<?php } ?>
				...
				<?php if ($this->config->item('enable_customer_loyalty_system') && isset($customer_points) && !$this->config->item('hide_points_on_receipt') && $this->config->item('loyalty_option') == 'advanced') {?>
					<tr>
						<td colspan="9" class="border-bottom border-top text-bold"><?php echo lang('common_points').': '; 
                                                echo NumberFormatToCurrency(to_quantity($customer_points)); ?></td>
					</tr>
				<?php } ?>
	...
	<div>
            <p>Số tiền viết bằng chữ: <span><?php echo getStringNumber($amount_money)?></span></p>
	</div>
