...
class BizCustomers extends Customers 
{
	...
	*/
	...
	function quotes_contract_delete() {
		...
		if ($this->Customer->delete_list($id)) {
	...
	function quotes_contract_search() {
		...
		$cat = $this->input->post('cat');
		$per_page = $this->config->item('number_of_items_per_page') ? (int) $this->config->item('number_of_items_per_page') : 20;
		$search_data = $this->Customer->search($search, $cat, $per_page, $this->input->post('offset') ? $this->input->post('offset') : 0, $this->input->post('order_col') ? $this->input->post('order_col') : 'id_quotes_contract', $this->input->post('order_dir') ? $this->input->post('order_dir') : 'desc');
		$config['base_url'] = site_url('quotes_contract/search');
		$config['total_rows'] = $this->Customer->search_count_all($search, $cat);
		$config['per_page'] = $per_page;
		$this->pagination->initialize($config);
		$data['pagination'] = $this->pagination->create_links();
	...
	function quotes_contract_view($id = -1) {
		$this->check_action_permission('add_update');
		$data = array();
		$data['info_quotes_contract'] = $this->Customer->get_info($id);
