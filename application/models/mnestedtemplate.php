<?php 
class MNestedTemplate extends CI_Model{
	...
	public function insertNode($data,$parent = 1, $template_id, $options = null){
		$this->_data        = $data;
		$this->_parent_id   = $parent;
	...
	protected function insertRight(){
		...
		$sqlUpdateLeft = 'UPDATE ' .$this->db->dbprefix($this->_table)
					  . ' SET lft = (lft + 2) '
					  . ' WHERE lft > ' . $parentRight . ' AND template_id = ' . $this->_template_id;
		...
		$sqlUpdateRight = 'UPDATE ' .$this->db->dbprefix($this->_table)
					   . ' SET rgt = (rgt + 2) '
					   . ' WHERE rgt >= ' . $parentRight . ' AND template_id = ' . $this->_template_id;
