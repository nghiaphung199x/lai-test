...
class Summary_categories extends Report
{
	...
	{
		$this->db->select('sales_items_temp.category, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax, sum(profit) as profit, sum(quantity_purchased) as item_sold', false);
		...
		{
			$this->db->where_in('category', $this->params['compare_to_categories']);
		}	
